...
def resolve_schema(schema, schemas):
    if isinstance(schema, dict):
        if '$ref' in schema:
            return resolve_ref(schema['$ref'], schemas)
        return {k: resolve_schema(v, schemas) for k, v in schema.items() if k != 'type'}
    elif isinstance(schema, list):
        return [resolve_schema(item, schemas) for item in schema]
    else:
        return schema
